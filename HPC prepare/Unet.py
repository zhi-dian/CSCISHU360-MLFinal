...
log(id_mask_count.info())
log(id_mask_count.head())
...
log(f"training set   {train_ids[:5]}.. with length {len(train_ids)}")
log(f"validation set {valid_ids[:5]}.. with length {len(valid_ids)}")
log(f"testing set    {test_ids[:5]}.. with length {len(test_ids)}")

# define dataset and dataloader
num_workers = 1
bs = 24

log(f"train with batch size : {bs}, number of Workers : {num_workers}")
...
for epoch in range(1, n_epochs+1):
    ...
    valid_loss = 0.0
    dice_score = 0.0
    log(f"Training {epoch} Start")
    model.train()
    bar = tq(train_loader, postfix={"train_loss":0.0})
    for data, target in bar:
        # move tensors to GPU if CUDA is available
        # log(data.size(),target.size())
        if train_on_gpu:
        ...
        # log(f"train epoch {epoch}: loss : {loss}")
    ######################    
    # validate the model #
    ######################
    model.eval()
    ...
    log(f"train loss: {train_loss}, valid loss: {valid_loss}, dice score: {dice_score}")
    ...
    if valid_loss <= valid_loss_min:
        log('Validation loss decreased ({:.6f} --> {:.6f}).  Saving model ...'.format(
        valid_loss_min,
        valid_loss))
...
for file in contents:
    if file == 'model_cifar.pt':
        log(f"Found model file at: {os.path.join(cwd, file)}")
...
log(f"test loss: {test_loss}, dice score: {dice_score}")
